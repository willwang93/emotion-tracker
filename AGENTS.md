...
Privacy-first mobile emotion tracker built with React Native and Expo SDK 57 for Android 14 (Google Pixel 5). Inspired by Yale's *How We Feel*, featuring Yale Mood Meter check-ins, SQLite persistence, audio speech recognition, and local notifications.
...
- **Audio & Speech:** `expo-speech-recognition` (^57.0.0) with Expo Go fallbacks
...
```
App.tsx                  # Root app layout, day selector, state orchestration
src/
├── components/          # UI components (QuadrantSelector, MicroCheckInModal,
│                        # SomaticSelector, ContextSelector, TimelineCard, ReminderModal)
├── constants/
...
│   ├── speech.ts        # expo-speech-recognition integration & safe Expo Go fallbacks
...
- **Guard Native Modules for Expo Go:** Modules requiring native code like `expo-speech-recognition` and Android `expo-notifications` must be guarded with `isRunningInExpoGo()` or `requireOptionalNativeModule` to prevent crashes when previewing in Expo Go.
- **Run Type Checks:** Always verify TypeScript compilation with `npx tsc --noEmit` before concluding work.
- **Maintain Dark Theme Consistency:** UI palette uses neutral dark slate (`#090D16`, `#111827`, `#1F2937`) with Mood Meter accent colors (`#EF4444`, `#F59E0B`, `#3B82F6`, `#10B981`).
